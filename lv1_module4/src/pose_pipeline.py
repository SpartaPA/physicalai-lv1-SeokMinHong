...
class PosePipeline:
    ...
    def __init__(self, T_base_link, T_link_camera, joint_axis: str = "z"):
        ...
        self._T_base_link0 = np.asarray(T_base_link, dtype=float)
        self._T_link_camera = np.asarray(T_link_camera, dtype=float)
        if self._T_base_link0.shape != (4, 4):
            raise ValueError("T_base_link 4x4 matrix여야 합니다.")
        elif self._T_link_camera.shape != (4, 4):
            raise ValueError("T_link_camera 4x4 matrix여야 합니다.")
