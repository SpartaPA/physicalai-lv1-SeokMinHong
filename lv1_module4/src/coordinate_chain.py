...
__all__ = ["CoordinateChain", "base_point_to_camera", "camera_point_to_base", "default_chain"]
...
class CoordinateChain:
    ...
    def _path_to_root(self, frame: str) -> list[str]:
        ...
        # TODO: 문제 6-1
        path = [frame]
        while self._parent.get(path[-1]) is not None:
            path.append(self._parent[path[-1]])
        if path[-1] != self.root:
            raise KeyError(f"{frame}은 루트와 연결되어 있지 않습니다.")
        return path
    ...
    def T_from_root(self, frame: str) -> np.ndarray:
        ...
        for index in range(len(path) - 1):
            result = self.get(path[index + 1], path[index]) @ result
        return result
    ...
    def transform(self, target: str, source: str, P, w: float = 1.0) -> np.ndarray:
        ...
        return transform_points(T_target_source, P, w)
