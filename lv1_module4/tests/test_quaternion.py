...
def test_slerp_endpoints(q_pair):
    ...
    q0, q1 = q_pair
    result_start = slerp(q0, q1, 0)
    result_goal = slerp(q0, q1, 1)
    assert np.isclose(np.abs(np.dot(result_start, q0)), 1.0)
    assert np.isclose(np.abs(np.dot(result_goal, q1)), 1.0)
