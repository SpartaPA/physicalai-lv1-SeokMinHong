...
setup(
    name=package_name,
    version='0.1.0',
    # 파이썬 모듈 디렉터리 (turtle_py/__init__.py 가 있는 폴더)
    packages=[package_name],
    # ------------------------------------------------------------------
    # data_files : 파이썬 코드가 "아닌" 파일을 install/ 아래 어디에 복사할지 지정.
    #  - resource/turtle_py : ament 인덱스 마커. 이 파일이 있어야
    #    `ros2 pkg list` 와 `ros2 run` 이 이 패키지를 찾습니다.
    #  - package.xml            : share/turtle_py/ 에 복사 (의존성 정보)
    #  - launch/*.launch.py     : share/turtle_py/launch/  ← 문제 9
    #  - config/*.yaml          : share/turtle_py/config/  ← 문제 9
    #    launch 파일은 get_package_share_directory('turtle_py') 로
    ...
    #   '실행파일이름 = 파이썬패키지.모듈:함수'
    #
    #  - 왼쪽 "실행파일이름" 이 `ros2 run turtle_py <실행파일이름>` 에 쓰이는 이름입니다.
    #  - 오른쪽은 "어떤 모듈의 어떤 함수를 호출할지" 입니다. 모든 예제는 main() 을 진입점으로 둡니다.
    #  - colcon build 가 끝나면 install/turtle_py/lib/turtle_py/ 아래에
    ...
    entry_points={
        'console_scripts': [
            ...
            'ex03_draw_shape = turtle_py.ex03_draw_shape:main',
            # 문제 5
