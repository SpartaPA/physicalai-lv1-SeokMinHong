...
- **문제 5~9 의 예제 코드**가 이 저장소의 `lv1_module2/` 에 주석과 함께 제공됩니다. 그대로 복사해 제출하면 인정되지 않으며, 본인의 `turtle_py` 패키지에 통합하고 실행 결과를 `report.md` 에 기록해야 합니다.
...
> 문제 5~9 는 **예제 코드가 제공**됩니다 (`lv1_module2/ros2_ws/src/turtle_examples/`, `turtle_interfaces/`). 예제는 주석으로 동작 원리를 설명하며, 여러분은 이를 본인 패키지에 통합하고 **실행·관찰·기록**을 해야 합니다. 채점은 기록(출력·캡처·표·설명)을 봅니다.
